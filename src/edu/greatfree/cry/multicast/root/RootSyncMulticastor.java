...
//	public RootSyncMulticastor(CryptoPeer<MulticastRootDispatcher> eventer, int rootBranchCount, int treeBranchCount, int cryptoOption)
	...
	public void addPartner(IPAddress ip) throws PeerNameIsNullException
	{
		this.eventer.addPeer(ip.getPeerName(), ip);
	}
	
	public void addPartners(Set<IPAddress> ips) throws PeerNameIsNullException
	{
		for (IPAddress entry : ips)
		{
			this.addPartner(entry);
		}
	}
				...
				for (String childrenKey : tree.get(UtilConfig.ROOT_KEY))
				{
					...
					if (allChildrenKeys != UtilConfig.NO_CHILDREN_KEYS)
					{
						...
						for (String childrenKeyInTree : allChildrenKeys)
						{
							...
							// The name is required to get IP from the registry. 02/11/2023, Bing Li
...
//			log.info("1) RootSyncMulticastor-notify(): notification is sent without children");
			notification.setChildrenNodes(UtilConfig.NO_IPS);
//			log.info("client keys' size = " + this.eventer.getClientKeys().size());
			// Send the message one by one to the immediate nodes of the root. 11/10/2014, Bing Li
...
//		if (this.receiverCounts.get(request.getCollaboratorKey()) > this.rootBranchCount)
		...
		{
			/*
			 * The root is represented by the ROOT_KEY, which does not represent any IP. So it does not matter whether it is ROOT_KEY or LOCAL_KEY. But it is required to avoid childrenKeys including the local IP, which results in data being sent to the local node. 02/15/2023, Bing Li
			 * 
			 * The ROOT_KEY is not correct for intercasting. For the intercasting, the child as the source needs to construct a tree. Thus, the ROOT_KEY is a fault in the case. 02/14/2023, Bing Li
			 */
			// Construct a tree if the count of nodes is larger than the capacity of the root. Without the tree, the root node has to send requests concurrently out of its capacity. To lower its load, the tree is required. All the nodes to received the multicast data are from the FreeClient pool. 11/28/2014, Bing Li
			tree = Tree.constructTree(UtilConfig.ROOT_KEY, new LinkedList<String>(childrenKeys), this.rootBranchCount, this.treeBranchCount);
			// Is sending exception existed. 05/18/2017, Bing Li
			...
			for (String childrenKey : tree.get(UtilConfig.ROOT_KEY))
			{
				...
				if (allChildrenKeys != UtilConfig.NO_CHILDREN_KEYS)
				{
					...
					for (String childrenKeyInTree : allChildrenKeys)
					{
						...
						log.info("IP name = " + ip.getPeerName());
						log.info("IP = " + ip);
						
						/*
						 * For intercasting, the root is located at one source child. Thus, the peer name retrieved might be null. 02/11/2023, Bing Li
						 */
						ip.setPeerName(this.eventer.getPartnerName(ip.getIPKey()));
						if (ip.getPeerName() == null)
						{
							log.info("******* peer name is NULL!");
						}
						// Retrieve the IP of a child node of the immediate child of the root and save the IP into the map. 11/28/2014, Bing Li
						remoteServerIPs.put(childrenKeyInTree, ip);
					}
