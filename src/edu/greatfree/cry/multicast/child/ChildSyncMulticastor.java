...
//	public ChildSyncMulticastor(Peer eventer, String localIPKey, int treeBranchCount, int cryptoOption)
	...
	public void notify(PrimitiveMulticastNotification notification) throws InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, SignatureException, InterruptedException, SymmetricKeyUnavailableException, CryptographyMismatchException, PublicKeyUnavailableException, DistributedNodeFailedException, IOException, ClassNotFoundException, RemoteReadException, RemoteIPNotExistedException, PeerNameIsNullException
	{
...
//			System.out.println("ChildEventer-disseminate(): NO children!");
	 ...
	 */
	public void read(PrimitiveMulticastRequest request) throws IOException, DistributedNodeFailedException, InvalidKeyException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException, SignatureException, InterruptedException, SymmetricKeyUnavailableException, CryptographyMismatchException, PublicKeyUnavailableException, ClassNotFoundException, RemoteReadException, RemoteIPNotExistedException, PeerNameIsNullException
	{
...
//								System.out.println("2) ChildSyncMulticastor-read(): send to " + childrenFromRoot.get(childrenKey));
			...
			else
			{
				...
				for (Map.Entry<String, IPAddress> entry : childrenFromRoot.entrySet())
				{
					if (!entry.getKey().equals(this.localIPKey))
					{
						log.info("child entry IP = " + entry.getValue().getIP());
						log.info("child entry Port = " + entry.getValue().getPort());
						try
						{
							// Send the message to the immediate node of the local node. 11/11/2014, Bing Li
